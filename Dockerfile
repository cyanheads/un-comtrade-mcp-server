...
# compiled native bindings — a dependency postinstall can otherwise abort the build.
...
ARG APP_VERSION
LABEL org.opencontainers.image.title="un-comtrade-mcp-server"
LABEL org.opencontainers.image.description="Access UN Comtrade international merchandise and services trade statistics via MCP."
...
# that are not needed in the final production image. --omit=peer keeps the framework's
# optional peer tiers (test runners, service SDKs) out of the runtime image.
...
RUN mkdir -p /var/log/un-comtrade-mcp-server && chown -R bun:bun /var/log/un-comtrade-mcp-server
...
ENV LOGS_DIR="/var/log/un-comtrade-mcp-server"
