...
Thanks for using `un-comtrade-mcp-server`. Bugs, feature requests, and documentation gaps all belong in an issue — that's where they get read and picked up.

Open a [Bug Report](https://github.com/cyanheads/un-comtrade-mcp-server/issues/new?template=bug_report.yml) or [Feature Request](https://github.com/cyanheads/un-comtrade-mcp-server/issues/new?template=feature_request.yml). Both are structured, and filling in the fields is what makes an issue actionable. Anything that fits neither can be a plain issue — a half-formed idea in your own words is fine.

## Server bug or framework bug?

`un-comtrade-mcp-server` is built on [@cyanheads/mcp-ts-core](https://github.com/cyanheads/mcp-ts-core), which handles transports, auth, config, logging, and telemetry. Sorting out which layer broke saves everyone a round-trip:

- **This repo** — a tool returns wrong data, a UN Comtrade API call fails, a schema doesn't match reality, a description misleads the model.
...
3. **Redact anything sensitive.** Issues are public and permanent — no API keys, tokens, auth headers, internal URLs, or PII in code, logs, or stack traces. `COMTRADE_SUBSCRIPTION_KEY` in particular should never appear in a pasted config or log line.
